# WCS - RMS 搬運控制與目的地變更流程設計 (A-01-2 到 repeat-A-0)

本設計文件根據 `00-2025-08-09_WCS-RMS 協議_登彥科技機密_V09_TBD.pdf` 的三階段回應原則（Request - Result - Ack），規劃 AMR 從 **A-01-2** 搬運貨物時，在取貨完成後由 WCS 下達 `replace` 控制指令變更卸貨終點為 **repeat-A-0** 的完整 API 交互步驟與對應的 JSON Payload。
...
- **原始任務動作序列：**
  ...
  3. `L-01-0`：`unload` (執行放貨) -> **任務執行中將被變更為 `repeat-A-0`**
  4. `L-01-0`：`end` (任務結束) -> **任務執行中將被變更為 `repeat-A-0`**
- **車輛名稱：** `AMR01`
- **指令序號：** `M1000001`
- **控制指令序號：** `M2000001`
- **棧板 ID：** `01`
...
### 步驟 7: WCS 下達任務狀態變更請求 (Status Request - replace)

* **方向：** WCS $\rightarrow$ RMS
* **API Endpoint：** `POST http://<RMS_IP>:31111/awd/rms/set_mission_status_request`
* **說明：** WCS 因調度需求，下達狀態變更指令為 `replace`，並重新指派剩餘的子任務目的地為 `repeat-A-0`。
...
### 步驟 8: RMS 立即回應狀態變更請求接收結果 (Status Reply)
...
### 步驟 10: WCS 確認收到狀態變更完成訊號 (Status Ack)
...
### 步驟 11: RMS 回報放貨完成 (Result - unload)
