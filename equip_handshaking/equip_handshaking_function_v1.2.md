...
| PalletSupply | 棧板供收機      | 1            | 進單板            | op_mode:1 (單板作業), 2:上層/子板作業                 | isOK/Wait, 最底層 palletNo                |
| PalletSupply | 棧板供收機      | 2            | 進滿板            | op_mode:1 (單板作業), 2:上層/子板作業                 | isOK/Wait, 最底層 palletNo                |
| PalletSupply | 棧板供收機      | 3            | 出單版            | op_mode:1 (單板作業), 2:上層/子板作業                 | isOK/Wait, 最底層 palletNo                |
| PalletSupply | 棧板供收機      | 4            | 出滿版            | op_mode:1 (單板作業), 2:上層/子板作業                 | isOK/Wait, 最底層 palletNo                |
...
| Station      | 手臂棧板位      | 1            | 入料到位感測      | -                                                     | isOK/Wait                                 |
| Station      | 手臂棧板位      | 2            | 出料釋放資源      | -                                                     | isOK/Wait                                 |
...
1.  參考上表，實作所有機台與目的模式的交握流程� ```python
    ...
    申請進入設備。
        ...
        :param kwargs: 機台特有額外引數 (如 pallet_no, cargo_height, rack_in_place, op_mode)
...
def request_enter(
    ...
    :param equipment_type: 設備類型 (Wrap, Check, Aligner, PalletSupply, Station)
    :param wes_id: WES_ID
...
def preparation_complete(equipment_type: str, wes_id: str, purpose_mode: int) -> str:
    ...
    match equipment_type:
        ...
        case "Station":
            match purpose_mode:
                case 1 | 2:
                    logger.info(f"  -> [Match-Case] [Station] 準備完成通知 (模式 {purpose_mode})，模擬等待 3 秒...")
                    time.sleep(3)
                    return "OK"
                case _:
                    logger.info(f"  -> [Match-Case] [Station] 準備完成通知 (未知模式 {purpose_mode})，模擬等待 3 秒...")
                    time.sleep(3)
                    return "OK"
...
- `equipment_type` (string): 設備類型 (`Wrap`, `Check`, `Aligner`, `PalletSupply`, `ASRS_IPORT`, `ASRS_OPORT`, `LIFT`, `Station`)。
